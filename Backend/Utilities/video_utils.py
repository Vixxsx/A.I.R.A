...
class VideoProcessor():
    ...
    def capture_video(self, duration: int = 10, camera_index: int = 0, filename: Optional[str] = None, fps: int = 30) -> str:
        ...
        print(f"🎥 Recording started... {width}x{height} @ {fps}fps for {duration}s")
            ...
            while frame_count < total_frames:
                ...
                if ret:
                    ...
                    if frame_count % (fps * 2) == 0:
                        print(f"   ⏱️  Recorded {frame_count // fps} seconds")
                else:
    ...
    def extract_frames(self, video_path: str, output_folder: Optional[str] = None, every_nth: int = 10, max_frames: Optional[int] = None, return_arrays: bool = True) -> List[np.ndarray]:
        ...
        print(f"📹 Extracting frames from: {os.path.basename(video_path)} (every {every_nth}th frame)")
        ...
        try:
            while True:
                ...
                if frame_index % every_nth == 0:
                    if output_folder:
                        frame_path = os.path.join(output_folder, f"frame_{frame_index:06d}.jpg")
        ...
        try:
            for timestamp in timestamps:
                ...
                if ret:
                    frames.append(frame)
                    if output_folder:
                        frame_path = os.path.join(output_folder, f"frame_at_{int(timestamp * 1000)}ms.jpg")
                        cv2.imwrite(frame_path, frame)
    ...
    def cleanup_temp_files(self, older_than_hours: int = 24):
        ...
        print(f"Cleaned up {deleted_count} old file(s)" if deleted_count > 0 else "No old files to clean up")
